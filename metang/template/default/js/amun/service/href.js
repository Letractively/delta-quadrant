/*
 *  $Id: href.js 117 2010-11-24 20:31:42Z k42b3.x $
 *
 * amun
...
psx.plugin.href.store = new Ext.data.Store({

	url: psx_url + '/index.php?x=admin/plugin/href',
	remoteSort: true,
...
psx.plugin.href.store_page = new Ext.data.Store({

	url: psx_url + '/index.php?x=admin/plugin/href/util/page',
	remoteSort: true,
	reader: new Ext.data.XmlReader({
...
psx.plugin.href.index = Ext.extend(psx.common.grid, {

	store: psx.plugin.href.store,
	columns: [
		...
		{header: 'href',    width: 350, dataIndex: 'href',    menuDisabled: true, sortable: true},
...
psx.plugin.href.form = Ext.extend(psx.common.form, {
	...
	fields: [{
		...
		fieldLabel: 'page',
		hiddenName: 'page_id',
		store: psx.plugin.href.store_page,
		valueField: 'value',
		displayField: 'name',
		editable: false,
		mode: 'remote',
...
psx.plugin.href.panel = Ext.extend(Ext.Panel, {
	...
	initComponent: function(){
		...
		this.o_index = new psx.plugin.href.index();
		this.o_form  = new psx.plugin.href.form();
		...
		psx.plugin.href.panel.superclass.initComponent.apply(this, arguments);
	...
	handler_index: function(){
		...
		this.o_index.getTopToolbar().get(0).on('click', function(){
			...
			this.o_form.url = psx_url + '/index.php?x=admin/plugin/href/add';
		...
		this.o_index.getTopToolbar().get(1).on('click', function(){
			...
			this.o_form.url = psx_url + '/index.php?x=admin/plugin/href/edit';

			this.o_form.load_data(psx_url + '/index.php?x=admin/plugin/href/util/item&id=' + this.o_index.row_id);
		...
		this.o_index.getTopToolbar().get(2).on('click', function(){
			...
			this.o_form.url = psx_url + '/index.php?x=admin/plugin/href/delete';
			...
			if(action.type == 'submit'){
				...
				if(msg != '')
				{
					Ext.MessageBox.alert('success', msg, function(btn) {
						...
						psx.plugin.href.store.load({params:{start:0, limit:16}});
