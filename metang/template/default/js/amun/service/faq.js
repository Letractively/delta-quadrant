/*
 *  $Id: faq.js 117 2010-11-24 20:31:42Z k42b3.x $
 *
 * amun
...
psx.plugin.faq.store = new Ext.data.Store({

	url: psx_url + '/index.php?x=admin/plugin/faq',
	remoteSort: true,
...
psx.plugin.faq.store_page = new Ext.data.Store({

	url: psx_url + '/index.php?x=admin/plugin/faq/util/page',
	remoteSort: true,
	reader: new Ext.data.XmlReader({
...
psx.plugin.faq.index = Ext.extend(psx.common.grid, {

	store: psx.plugin.faq.store,
	columns: [
		...
		{header: 'question', width: 350, dataIndex: 'question', menuDisabled: true, sortable: true},
...
psx.plugin.faq.form = Ext.extend(psx.common.form, {
	...
	fields: [{
		...
		fieldLabel: 'page',
		hiddenName: 'page_id',
		store: psx.plugin.faq.store_page,
		valueField: 'value',
		displayField: 'name',
		editable: false,
		mode: 'remote',
...
psx.plugin.faq.panel = Ext.extend(Ext.Panel, {
	...
	initComponent: function(){
		...
		this.o_index = new psx.plugin.faq.index();
		this.o_form  = new psx.plugin.faq.form();
		...
		psx.plugin.faq.panel.superclass.initComponent.apply(this, arguments);
	...
	handler_index: function(){
		...
		this.o_index.getTopToolbar().get(0).on('click', function(){
			...
			this.o_form.url = psx_url + '/index.php?x=admin/plugin/faq/add';
		...
		this.o_index.getTopToolbar().get(1).on('click', function(){
			...
			this.o_form.url = psx_url + '/index.php?x=admin/plugin/faq/edit';

			this.o_form.load_data(psx_url + '/index.php?x=admin/plugin/faq/util/item&id=' + this.o_index.row_id);
		...
		this.o_index.getTopToolbar().get(2).on('click', function(){
			...
			this.o_form.url = psx_url + '/index.php?x=admin/plugin/faq/delete';
			...
			if(action.type == 'submit'){
				...
				if(msg != '')
				{
					Ext.MessageBox.alert('success', msg, function(btn) {
						...
						psx.plugin.faq.store.load({params:{start:0, limit:16}});
