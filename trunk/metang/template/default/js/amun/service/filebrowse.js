/*
 *  $Id: filebrowse.js 117 2010-11-24 20:31:42Z k42b3.x $
 *
 * amun
...
Ext.ns('psx.plugin.filebrowse');

/**
 * psx.plugin.filebrowse
 *
...
psx.plugin.filebrowse.store = new Ext.data.Store({

	url: psx_url + '/index.php?x=admin/plugin/filebrowse',
	remoteSort: true,
...
psx.plugin.filebrowse.store_page = new Ext.data.Store({

	url: psx_url + '/index.php?x=admin/plugin/filebrowse/util/page',
	remoteSort: true,
	reader: new Ext.data.XmlReader({
...
psx.plugin.filebrowse.index = Ext.extend(psx.common.grid, {

	store: psx.plugin.filebrowse.store,
	columns: [
...
psx.plugin.filebrowse.form = Ext.extend(psx.common.form, {
	...
	fields: [{
		...
		fieldLabel: 'page',
		hiddenName: 'page_id',
		store: psx.plugin.filebrowse.store_page,
		valueField: 'value',
		displayField: 'name',
		editable: false,
		mode: 'remote',
...
psx.plugin.filebrowse.panel = Ext.extend(Ext.Panel, {
	...
	initComponent: function(){
		...
		this.o_index = new psx.plugin.filebrowse.index();
		this.o_form  = new psx.plugin.filebrowse.form();
		...
		psx.plugin.filebrowse.panel.superclass.initComponent.apply(this, arguments);
	...
	handler_index: function(){
		...
		this.o_index.getTopToolbar().get(0).on('click', function(){
			...
			this.o_form.url = psx_url + '/index.php?x=admin/plugin/filebrowse/add';
		...
		this.o_index.getTopToolbar().get(1).on('click', function(){
			...
			this.o_form.url = psx_url + '/index.php?x=admin/plugin/filebrowse/edit';

			this.o_form.load_data(psx_url + '/index.php?x=admin/plugin/filebrowse/util/item&id=' + this.o_index.row_id);
		...
		this.o_index.getTopToolbar().get(2).on('click', function(){
			...
			this.o_form.url = psx_url + '/index.php?x=admin/plugin/filebrowse/delete';
			...
			if(action.type == 'submit'){
				...
				if(msg != '')
				{
					Ext.MessageBox.alert('success', msg, function(btn) {
						...
						psx.plugin.filebrowse.store.load({params:{start:0, limit:16}});
