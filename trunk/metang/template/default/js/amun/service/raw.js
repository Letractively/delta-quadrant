/*
 *  $Id: raw.js 117 2010-11-24 20:31:42Z k42b3.x $
 *
 * amun
...
psx.plugin.raw.store = new Ext.data.Store({

	url: psx_url + '/index.php?x=admin/plugin/raw',
	remoteSort: true,
	reader: new Ext.data.XmlReader({
		...
		{name: 'last_mod', type: 'date',    dateFormat: 'Y-m-d H:i:s'},
...
psx.plugin.raw.store_page = new Ext.data.Store({

	url: psx_url + '/index.php?x=admin/plugin/raw/util/page',
	remoteSort: true,
	reader: new Ext.data.XmlReader({
...
psx.plugin.raw.index = Ext.extend(psx.common.grid, {

	store: psx.plugin.raw.store,
	columns: [
...
psx.plugin.raw.form = Ext.extend(psx.common.form, {
	...
	fields: [{
		...
		fieldLabel: 'page',
		hiddenName: 'page_id',
		store: psx.plugin.raw.store_page,
		valueField: 'value',
		displayField: 'name',
		editable: false,
		mode: 'remote',
...
psx.plugin.raw.panel = Ext.extend(Ext.Panel, {
	...
	initComponent: function(){
		...
		this.o_index = new psx.plugin.raw.index();
		this.o_form  = new psx.plugin.raw.form();
		...
		psx.plugin.raw.panel.superclass.initComponent.apply(this, arguments);
	...
	handler_index: function(){
		...
		this.o_index.getTopToolbar().get(0).on('click', function(){
			...
			this.o_form.url = psx_url + '/index.php?x=admin/plugin/raw/add';
		...
		this.o_index.getTopToolbar().get(1).on('click', function(){
			...
			this.o_form.url = psx_url + '/index.php?x=admin/plugin/raw/edit';

			this.o_form.load_data(psx_url + '/index.php?x=admin/plugin/raw/util/item&id=' + this.o_index.row_id);
		...
		this.o_index.getTopToolbar().get(2).on('click', function(){
			...
			this.o_form.url = psx_url + '/index.php?x=admin/plugin/raw/delete';
			...
			if(action.type == 'submit'){
				...
				if(msg != '')
				{
					Ext.MessageBox.alert('success', msg, function(btn) {
						...
						psx.plugin.raw.store.load({params:{start:0, limit:16}});
