/*
 *  $Id: contact.js 117 2010-11-24 20:31:42Z k42b3.x $
 *
 * amun
...
psx.plugin.contact.store = new Ext.data.Store({

	url: psx_url + '/index.php?x=admin/plugin/contact',
	remoteSort: true,
...
psx.plugin.contact.store_page = new Ext.data.Store({

	url: psx_url + '/index.php?x=admin/plugin/contact/util/page',
	remoteSort: true,
	reader: new Ext.data.XmlReader({
...
psx.plugin.contact.index = Ext.extend(psx.common.grid, {

	store: psx.plugin.contact.store,
	columns: [
...
psx.plugin.contact.form = Ext.extend(psx.common.form, {
	...
	fields: [{
		...
		fieldLabel: 'page',
		hiddenName: 'page_id',
		store: psx.plugin.contact.store_page,
		valueField: 'value',
		displayField: 'name',
		editable: false,
		mode: 'remote',
...
psx.plugin.contact.panel = Ext.extend(Ext.Panel, {
	...
	initComponent: function(){
		...
		this.o_index = new psx.plugin.contact.index();
		this.o_form  = new psx.plugin.contact.form();
		...
		psx.plugin.contact.panel.superclass.initComponent.apply(this, arguments);
	...
	handler_index: function(){
		...
		this.o_index.getTopToolbar().get(0).on('click', function(){
			...
			this.o_form.url = psx_url + '/index.php?x=admin/plugin/contact/add';
		...
		this.o_index.getTopToolbar().get(1).on('click', function(){
			...
			this.o_form.url = psx_url + '/index.php?x=admin/plugin/contact/edit';

			this.o_form.load_data(psx_url + '/index.php?x=admin/plugin/contact/util/item&id=' + this.o_index.row_id);
		...
		this.o_index.getTopToolbar().get(2).on('click', function(){
			...
			this.o_form.url = psx_url + '/index.php?x=admin/plugin/contact/delete';
			...
			if(action.type == 'submit'){
				...
				if(msg != '')
				{
					Ext.MessageBox.alert('success', msg, function(btn) {
						...
						psx.plugin.contact.store.load({params:{start:0, limit:16}});
