/*
 *  $Id: gallery.js 117 2010-11-24 20:31:42Z k42b3.x $
 *
 * amun
...
psx.plugin.gallery.store = new Ext.data.Store({

	url: psx_url + '/index.php?x=admin/plugin/gallery',
	remoteSort: true,
...
psx.plugin.gallery.store_page = new Ext.data.Store({

	url: psx_url + '/index.php?x=admin/plugin/gallery/util/page',
	remoteSort: true,
	reader: new Ext.data.XmlReader({
...
psx.plugin.gallery.index = Ext.extend(psx.common.grid, {

	store: psx.plugin.gallery.store,
	columns: [
...
psx.plugin.gallery.form = Ext.extend(psx.common.form, {
	...
	fields: [{
		...
		fieldLabel: 'page',
		hiddenName: 'page_id',
		store: psx.plugin.gallery.store_page,
		valueField: 'value',
		displayField: 'name',
		editable: false,
		mode: 'remote',
...
psx.plugin.gallery.panel = Ext.extend(Ext.Panel, {
	...
	initComponent: function(){
		...
		this.o_index = new psx.plugin.gallery.index();
		this.o_form  = new psx.plugin.gallery.form();
		...
		psx.plugin.gallery.panel.superclass.initComponent.apply(this, arguments);
	...
	handler_index: function(){
		...
		this.o_index.getTopToolbar().get(0).on('click', function(){
			...
			this.o_form.url = psx_url + '/index.php?x=admin/plugin/gallery/add';
		...
		this.o_index.getTopToolbar().get(1).on('click', function(){
			...
			this.o_form.url = psx_url + '/index.php?x=admin/plugin/gallery/edit';

			this.o_form.load_data(psx_url + '/index.php?x=admin/plugin/gallery/util/item&id=' + this.o_index.row_id);
		...
		this.o_index.getTopToolbar().get(2).on('click', function(){
			...
			this.o_form.url = psx_url + '/index.php?x=admin/plugin/gallery/delete';
			...
			if(action.type == 'submit'){
				...
				if(msg != '')
				{
					Ext.MessageBox.alert('success', msg, function(btn) {
						...
						psx.plugin.gallery.store.load({params:{start:0, limit:16}});
